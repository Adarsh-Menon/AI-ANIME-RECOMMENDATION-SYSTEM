...
#DEFINE EVALUATERS (LLM AS A JUDGE--METRICS,RELEVANCY, FAITHFULLNESS)

def llm_relevance_evaluator(run, example):
    """
    LLM-as-a-Judge: Semantically grades the recommendation quality against a ground-truth reference 
    output and context.
    ...
    reference_output = example.output.get("expected_output", "")
    reference_context = example.output.get("context", "")
    
    #Using the same model as a judge
    judge_llm = init_chat_model("groq:qwen/qwen3.6-27b")
